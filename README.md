...
* Distribute the Kubernetes Configuration Files : Copy the appropriate kube-proxy kubeconfig files to each worker instance:

1. for instance in 192.168.1.{116,117}; do
  scp kube-proxy.kubeconfig ${instance}:/etc/kubernetes/
done

2. Copy the appropriate admin.kubeconfig, kube-controller-manager and kube-scheduler kubeconfig files to each controller instance:

for instance in 192.168.1.{114,115}; do
  scp admin.kubeconfig kube-controller-manager.kubeconfig kube-scheduler.kubeconfig ${instance}:
done

####################################################################################################################################################

# 2. Generating the Data Encryption Config and Key
...
# 3. Kubernetes release
  
  https://github.com/kubernetes/kubernetes/releases/tag/v1.27.11/

  ```
  wget https://github.com/kubernetes/kubernetes/archive/refs/tags/v1.27.11.zip
  unzip v1.27.11.zip
  cd kubernetes-1.27.11/
  ./cluster/get-kube.sh
...
ExecStart=/usr/local/bin/etcd \\
  ...
  --client-cert-auth \\
  --initial-advertise-peer-urls https://192.168.1.115:2380 \\
  --listen-peer-urls https://192.168.1.115:2380 \\
  --listen-client-urls https://192.168.1.115:2379,https://127.0.0.1:2379 \\
  --advertise-client-urls https://192.168.1.115:2379 \\
  --initial-cluster-token etcd-cluster-0 \\
  --initial-cluster master-1=https://192.168.1.114:2380,master-2=https://192.168.1.115:2380 \\
